...
- Locators and page logic live in Page Objects (`pages/`); shared setup/teardown lives in fixtures (`fixtures/`); reusable helpers/constants live in `utils/`
...
- **`tests/ui/`** — New UI tests always go here, named `<feature>.spec.ts`
- **`tests/api/`** — New API tests always go here, named `<feature>Api.spec.ts`
- **`tests/performance/`** — Load/performance test specs
- **`pages/`** — Every UI test must use a Page Object from here instead of inline selectors. Never call `page.locator()` or `page.click()` directly in a spec file — those belong inside Page Object methods (e.g. `navigate()`, `fillForm()`).
- **`fixtures/`** — Shared setup/teardown goes in a fixture here, not repeated per test with `beforeEach`
- **`utils/`** — Helper functions and constants shared across tests
- **`config/`** — Any config value (base URL, environment name, etc.) is read from here — never hardcoded in a test
- **`test-data/`** — Static test data lives in `test-data/*.json`, referenced by import — not inlined in the spec
- **`reports/`** / **`logs/`** — Generated output only; not hand-authored
